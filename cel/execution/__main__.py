...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"clock={algo_cfg.clock} min_move={algo_cfg.signal_move}"
    )
    inv = ", ".join(f"{k}: {v:.6f}" for k, v in state.inventory.items())
    print(
        f"fills={len(state.fills)} net={state.imbalance:.6f} max_venue={state.max_venue_abs:.6f} "
        f"inv={{{inv}}} killed={state.killed} pnl_usdt={pnl:.4f}"
    )
    return 0
